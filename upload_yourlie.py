# -*- coding: utf-8 -*-
"""
Your Lie in April / Shigatsu wa Kimi no Uso S01 (22 ep, VOSTFR, BD 1080p HEVC Opus)
-> R2 + genere your-lie-videos.json.
Source HEVC + audio Opus + sous-titres ASS (fre) -> TRANSCODE :
  video h265 -> h264 NVENC, audio opus -> aac, 1 MP4 VOSTFR/ep + sous-titres FR (vtt)
  + 1 miniature jpg/ep.
Resumable (head_object + temp local). Lancer : py upload_yourlie.py
"""
...
TMP=Path(r'F:\yourlie_tmp'); TMP.mkdir(parents=True,exist_ok=True)
SRC=Path(r'F:\Brams-Score-By-Freydiss-new\public\anime\Shigatsu wa Kimi no Uso - S01 VOSTFR [BD 1080p Opus] (Darki)')
JSON=Path(r'C:\Users\Feydi\Desktop\brams-web-clone\src\data\your-lie-videos.json')
KEY_PREFIX='anime/your-lie'
...
def main():
    ...
    print('Your Lie in April :',len(files),'episodes')
    entries=[]
    if JSON.exists():
    ...
    for ep in sorted(files):
        ...
        e={'episode':ep,'title':f'Épisode {ep}','episodeLabel':base,'src':url_vo,
           'season':'S01','arc':'Saison 1','preferredAudioLang':'ja','progressKey':base,'badge':'VOSTFR',
