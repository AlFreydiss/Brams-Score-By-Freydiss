...
export function useProfileData(discordId) {
  ...
  const load = useCallback((silent = false) => {
    ...
    // DÉCOUPLÉ : l'inventaire public (RPC dédiée, fiable) ne dépend plus de
    // fetchBerryShopState (RLS/auth, fragile) — son échec affichait « coffre
    // vide » alors que la base contenait bien les achats.
    fetchBerryShopState(discordId)
      .then(s => { if (!ignore && s) setShopData(prev => ({ ...s, inventory: prev?.inventory?.length ? prev.inventory : (s.inventory || []) })) })
      .catch(() => {})
    fetchMemberInventory(discordId)
      .then(inv => { if (!ignore && inv.length) setShopData(prev => ({ ...(prev || {}), inventory: inv })) })
      .catch(() => {})
