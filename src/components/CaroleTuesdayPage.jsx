...
const COLOR  = '#22d3ee'
const COLOR2 = '#67e8f9'
const NS     = 'carole-tuesday'

const SYNOPSIS = "Année 2099. Sur Mars, la musique est presque entièrement produite par des intelligences artificielles. Deux jeunes femmes que tout oppose — Carole, une chanteuse des rues au talent brut, et Tuesday, une fille de bonne famille qui a fui son foyer — se rencontrent par hasard. Ensemble, elles forment le duo Carole & Tuesday et décident de percer en misant sur ce que les IA ne pourront jamais reproduire : l'émotion et l'authenticité humaines."

const TAGS = ['Musique', 'Sci-Fi', 'Drame', 'Tranche de vie', 'Watanabe Shinichirō']

const AWARDS = [
  { icon: '🎤', label: 'Musique 100% humaine' },
  { icon: '🚀', label: 'Création de Shinichirō Watanabe' },
  { icon: '🎸', label: 'Chef-d\'œuvre musical futuriste' },
]

const COVER = '/carole-tuesday-poster.jpg'
...
const CSS = `
  ...
  @keyframes ctPulse { 0%,100%{opacity:.6} 50%{opacity:1} }
  ...
  .ct-ep-card:hover {
    ...
    box-shadow: 0 18px 48px rgba(34,211,238,.18) !important;
    border-color: rgba(34,211,238,.45) !important;
  }
  .ct-ep-card:focus-visible { outline: 2px solid ${COLOR}; outline-offset: 3px; }

  .ct-play-btn { transition: transform .16s, opacity .16s, background .16s; }
  .ct-ep-card:hover .ct-play-btn { transform: scale(1.15) !important; opacity: 1 !important; }

  .ct-cta { transition: background .18s, transform .14s, box-shadow .18s; }
  .ct-cta:hover {
  ...
  .ct-scroll { scrollbar-width: thin; scrollbar-color: rgba(34,211,238,.2) transparent; }
  .ct-scroll::-webkit-scrollbar { width: 4px; }
  .ct-scroll::-webkit-scrollbar-thumb { background: rgba(34,211,238,.2); border-radius: 4px; }
`
...
const EpCard = memo(function EpCard({ video, index, watched, onPlay }) {
  ...
  return (
    <div className="ct-ep-card" role="button" tabIndex={0} onClick={onPlay} onKeyDown={e => e.key === 'Enter' && onPlay()}
      style={{ borderRadius: 14, overflow: 'hidden', background: 'rgba(14,12,24,.92)', border: `1px solid ${watched ? 'rgba(103,232,249,.28)' : 'rgba(255,255,255,.07)'}`, animation: `ctFadeUp .3s ${index * 0.03}s ease-out both`, position: 'relative' }}>
      <div style={{ position: 'relative', paddingTop: '57%', background: '#0a0814', overflow: 'hidden' }}>
        {video.thumbnail && !imgErr
          ...
          : <div style={{ position:'absolute',inset:0, background:`linear-gradient(135deg,rgba(103,232,249,.12),rgba(0,0,0,.9))`, display:'flex',alignItems:'center',justifyContent:'center' }}>
              <span style={{ fontFamily:"'Pirata One',cursive", fontSize:40, fontWeight:900, color:`rgba(103,232,249,.28)`, lineHeight:1 }}>{fallbackLabel}</span>
            </div>}
        <div style={{ position:'absolute',inset:'40% 0 0', background:'linear-gradient(180deg,transparent,rgba(0,0,0,.65))', pointerEvents:'none' }} />
        <div className="ct-play-btn" style={{ position:'absolute',inset:0, display:'flex',alignItems:'center',justifyContent:'center', opacity: watched ? 0.5 : 0.78 }}>
          <div style={{ width:40,height:40,borderRadius:'50%',background:`rgba(34,211,238,.82)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,backdropFilter:'blur(6px)',boxShadow:`0 4px 18px rgba(34,211,238,.45)` }}>▶</div>
        </div>
        ...
        <div style={{ position:'absolute',bottom:8,left:8, fontSize:9,fontWeight:800,background:'rgba(103,232,249,.16)',color:COLOR2,border:`1px solid rgba(103,232,249,.28)`,borderRadius:100,padding:'2px 7px' }}>{video.badge || 'VOSTFR'}</div>
      </div>
...
function InfoPanel({ watchedCount, total, lastWatchedIdx, onResume }) {
  ...
  return (
    <aside style={{ position: 'sticky', top: 0, alignSelf: 'start', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(180deg,rgba(16,12,30,.96),rgba(10,8,20,.99))', border: '1px solid rgba(34,211,238,.18)', boxShadow: '0 24px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)', backdropFilter: 'blur(20px)' }}>
      <div style={{ position:'relative', height:260, overflow:'hidden', flexShrink:0 }}>
        <img src={COVER} alt="Carole & Tuesday" style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 15%',opacity:.72,filter:'saturate(1.1) brightness(.88)' }} />
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 20px' }}>
          <div style={{ fontSize:9.5,fontWeight:800,letterSpacing:'.18em',color:COLOR2,textTransform:'uppercase',marginBottom:6 }}>♫ RÊVE · AMITIÉ · MUSIQUE</div>
          <h2 style={{ margin:0, fontFamily:"'Pirata One',cursive", fontSize:22,fontWeight:900,color:'#fff',lineHeight:1.0,letterSpacing:'-.01em',textShadow:'0 2px 20px rgba(0,0,0,.9)' }}>Carole & Tuesday</h2>
          <div style={{ fontSize:11,color:'rgba(255,255,255,.4)',fontWeight:600,marginTop:3 }}>MAPPA · 2019</div>
        </div>
      </div>
      ...
      <div style={{ padding:'18px 18px 22px', display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <button className="ct-cta" onClick={onResume} style={{ width:'100%',padding:'11px 0',borderRadius:12, background:`rgba(34,211,238,.14)`,border:`1px solid rgba(34,211,238,.32)`, color:'#fff',cursor:'pointer',fontSize:13,fontWeight:800, display:'flex',alignItems:'center',justifyContent:'center',gap:8, fontFamily:'var(--body)' }}>
        ...
        <div style={{ display:'flex',flexWrap:'wrap',gap:5 }}>
          {TAGS.map(tag => (
            <span key={tag} style={{ fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:999,background:'rgba(34,211,238,.08)',border:'1px solid rgba(34,211,238,.2)',color:COLOR2 }}>{tag}</span>
          ))}
        </div>
...
export default function CaroleTuesdayPage({ onClose }) {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{ position:'fixed',inset:0,zIndex:500, background:'radial-gradient(circle at 18% 12%,rgba(34,211,238,.09),transparent 32rem),radial-gradient(circle at 84% 80%,rgba(103,232,249,.07),transparent 28rem),linear-gradient(135deg,#0e0a1a 0%,#100c1c 55%,#0a0814 100%)', display:'flex',flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0,height:62,padding:'0 24px', display:'flex',alignItems:'center',justifyContent:'space-between', background:'rgba(14,10,26,.96)',backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(34,211,238,.10)',zIndex:10, position:'relative' }}>
          ...
          <div style={{ position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:10 }}>
            ...
            <span style={{ fontFamily:"'Pirata One',cursive",fontSize:17,fontWeight:900,color:'#fff',letterSpacing:'-.01em' }}>
              {playerIdx !== null ? (VIDEOS[playerIdx]?.kind === 'film' || VIDEOS[playerIdx]?.kind === 'ova' ? VIDEOS[playerIdx]?.title : `Épisode ${VIDEOS[playerIdx]?.episode}`) : 'Carole & Tuesday'}
            </span>
          </div>
        ...
        {playerIdx !== null ? (
          ...
          <div ref={scrollRef} className="ct-scroll" style={{ flex:1,overflowY:'auto',padding:'24px 28px 48px' }}>
            <style>{`
              .ct-layout { display: grid; grid-template-columns: 310px minmax(0,1fr); gap: 28px; max-width: 1480px; margin: 0 auto; align-items: start; }
            ...
            <div className="ct-layout">
              ...
              <div>
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20 }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:999,background:'rgba(34,211,238,.08)',border:'1px solid rgba(34,211,238,.18)' }}>
                    <div style={{ width:6,height:6,borderRadius:'50%',background:watchedCount===VIDEOS.length?'#34d399':COLOR,animation:watchedCount<VIDEOS.length&&watchedCount>0?'ctPulse 2s infinite':'none' }} />
