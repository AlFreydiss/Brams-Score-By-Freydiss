...
const SYNOPSIS = "Sakuta Azusagawa vit des situations surnaturelles liées au 'Syndrome de l'Adolescence'. Quand il rencontre Mai Sakurajima, une actrice célèbre qui a commencé à 'disparaître' aux yeux des autres, il décide de l'aider à résoudre son mystère tout en affrontant ses propres démons émotionnels."

const TAGS = ['Romance', 'Drame', 'Surnaturel', 'Psychologique', 'Slice of Life']

const AWARDS = [
  { icon: '🐰', label: 'Bunny Girl & Science' },
  { icon: '🌙', label: 'Syndrome de l\'Adolescence' },
  { icon: '💫', label: 'Moments poignants' },
]

const COVER = '/bunny-girl-poster.jpg'
...
const CSS = `
  ...
  @keyframes bgFadeUp { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:none} }
  ...
  .bg-ep-card:hover {
    ...
    box-shadow: 0 18px 48px rgba(139,92,246,.18) !important;
    border-color: rgba(139,92,246,.45) !important;
  }
  .bg-ep-card:focus-visible { outline: 2px solid ${COLOR}; outline-offset: 3px; }

  .bg-play-btn { transition: transform .16s, opacity .16s, background .16s; }
  .bg-ep-card:hover .bg-play-btn { transform: scale(1.15) !important; opacity: 1 !important; }

  .bg-cta { transition: background .18s, transform .14s, box-shadow .18s; }
  .bg-cta:hover {
  ...
  .bg-scroll { scrollbar-width: thin; scrollbar-color: rgba(139,92,246,.2) transparent; }
  .bg-scroll::-webkit-scrollbar { width: 4px; }
  .bg-scroll::-webkit-scrollbar-thumb { background: rgba(139,92,246,.2); border-radius: 4px; }
`
...
const EpCard = memo(function EpCard({ video, index, watched, onPlay }) {
  ...
  return (
    <div className="bg-ep-card" role="button" tabIndex={0} onClick={onPlay} onKeyDown={e => e.key === 'Enter' && onPlay()}
      style={{ borderRadius: 14, overflow: 'hidden', background: 'rgba(14,12,24,.92)', border: `1px solid ${watched ? 'rgba(192,132,252,.28)' : 'rgba(255,255,255,.07)'}`, animation: `bgFadeUp .3s ${index * 0.03}s ease-out both`, position: 'relative' }}>
      <div style={{ position: 'relative', paddingTop: '57%', background: '#0a0814', overflow: 'hidden' }}>
        {video.thumbnail && !imgErr
          ...
          : <div style={{ position:'absolute',inset:0, background:`linear-gradient(135deg,rgba(139,92,246,.12),rgba(0,0,0,.9))`, display:'flex',alignItems:'center',justifyContent:'center' }}>
              <span style={{ fontFamily:"'Pirata One',cursive", fontSize:40, fontWeight:900, color:`rgba(139,92,246,.28)`, lineHeight:1 }}>{fallbackLabel}</span>
            </div>}
        <div style={{ position:'absolute',inset:'40% 0 0', background:'linear-gradient(180deg,transparent,rgba(0,0,0,.65))', pointerEvents:'none' }} />
        <div className="bg-play-btn" style={{ position:'absolute',inset:0, display:'flex',alignItems:'center',justifyContent:'center', opacity: watched ? 0.5 : 0.78 }}>
          <div style={{ width:40,height:40,borderRadius:'50%',background:`rgba(139,92,246,.82)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,backdropFilter:'blur(6px)',boxShadow:`0 4px 18px rgba(139,92,246,.45)` }}>▶</div>
        </div>
        ...
        <div style={{ position:'absolute',bottom:8,left:8, fontSize:9,fontWeight:800,background:'rgba(139,92,246,.16)',color:COLOR2,border:`1px solid rgba(139,92,246,.28)`,borderRadius:100,padding:'2px 7px' }}>{video.badge || 'VOSTFR'}</div>
      </div>
...
function InfoPanel({ watchedCount, total, lastWatchedIdx, onResume, chapterCount }) {
  ...
  return (
    <aside style={{ position: 'sticky', top: 0, alignSelf: 'start', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(180deg,rgba(16,12,30,.96),rgba(10,8,20,.99))', border: '1px solid rgba(139,92,246,.18)', boxShadow: '0 24px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)', backdropFilter: 'blur(20px)' }}>
      <div style={{ position:'relative', height:260, overflow:'hidden', flexShrink:0 }}>
        <img src={COVER} alt="Bunny Girl Senpai" style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 20%',opacity:.72,filter:'saturate(1.1) brightness(.9)' }} />
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 20px' }}>
          <div style={{ fontSize:9.5,fontWeight:800,letterSpacing:'.18em',color:COLOR2,textTransform:'uppercase',marginBottom:6 }}>🐰 CŒUR • SCIENCE • MYSTÈRE</div>
          <h2 style={{ margin:0, fontFamily:"'Pirata One',cursive", fontSize:22,fontWeight:900,color:'#fff',lineHeight:1.0,letterSpacing:'-.01em',textShadow:'0 2px 20px rgba(0,0,0,.9)' }}>Bunny Girl Senpai</h2>
          <div style={{ fontSize:11,color:'rgba(255,255,255,.4)',fontWeight:600,marginTop:3 }}>2018</div>
        </div>
      </div>
      ...
      <div style={{ padding:'18px 18px 22px', display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <button className="bg-cta" onClick={onResume} style={{ width:'100%',padding:'11px 0',borderRadius:12, background:`rgba(139,92,246,.14)`,border:`1px solid rgba(139,92,246,.32)`, color:'#fff',cursor:'pointer',fontSize:13,fontWeight:800, display:'flex',alignItems:'center',justifyContent:'center',gap:8, fontFamily:'var(--body)' }}>
        ...
        <div style={{ display:'flex',flexWrap:'wrap',gap:5 }}>
          {TAGS.map(tag => (
            <span key={tag} style={{ fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:999,background:'rgba(139,92,246,.08)',border:'1px solid rgba(139,92,246,.2)',color:COLOR2 }}>{tag}</span>
          ))}
        </div>
...
export default function BunnyGirlPage({ onClose }) {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{ position:'fixed',inset:0,zIndex:500, background:'radial-gradient(circle at 18% 12%,rgba(139,92,246,.09),transparent 32rem),radial-gradient(circle at 84% 80%,rgba(192,132,252,.07),transparent 28rem),linear-gradient(135deg,#0e0a1a 0%,#100c1c 55%,#0a0814 100%)', display:'flex',flexDirection:'column' }}>
        <AnimeBackdrop motifs={ANIME_MOTIFS['bunny-girl']} color={COLOR} color2={COLOR2} />
        {/* Navbar */}
        <div style={{ flexShrink:0,height:62,padding:'0 24px', display:'flex',alignItems:'center',justifyContent:'space-between', background:'rgba(14,10,26,.96)',backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(139,92,246,.10)',zIndex:10, position:'relative' }}>
          ...
          <div style={{ position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:10 }}>
            <span style={{ fontSize:17,animation:'bgFloat 6s ease-in-out infinite' }}>🐰</span>
            <span style={{ fontFamily:"'Pirata One',cursive",fontSize:17,fontWeight:900,color:'#fff',letterSpacing:'-.01em' }}>
              {playerIdx !== null ? (VIDEOS[playerIdx]?.kind === 'film' || VIDEOS[playerIdx]?.kind === 'ova' ? VIDEOS[playerIdx]?.title : `Épisode ${VIDEOS[playerIdx]?.episode}`) : 'Bunny Girl Senpai'}
            </span>
          </div>
        ...
        {playerIdx !== null ? (
          ...
          <div ref={scrollRef} className="bg-scroll" style={{ flex:1,overflowY:'auto',padding:'24px 28px 48px' }}>
            <style>{`
              .bg-layout { display: grid; grid-template-columns: 310px minmax(0,1fr); gap: 28px; max-width: 1480px; margin: 0 auto; align-items: start; }
              @media (max-width: 900px) { .bg-layout { grid-template-columns: 1fr; } }
            `}</style>
              ...
              <div>
                ...
                <div style={{ marginTop:28,padding:'14px 18px',borderRadius:12,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',gap:10 }}>
                  ...
                  <span style={{ fontSize:12,color:'rgba(255,255,255,.38)',fontWeight:600,lineHeight:1.5 }}>
                    Rascal Does Not Dream of Bunny Girl Senpai — une romance surnaturelle touchante.
                  </span>
                </div>
              </div>
            </div>
