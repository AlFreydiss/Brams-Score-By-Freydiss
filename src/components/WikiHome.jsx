...
const WH_CSS = `
  ...
  @keyframes whTwinkle { 0%,100% { opacity:.15 } 50% { opacity:.7 } }
  ...
  return (
    <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:1, overflow:'hidden' }}>
      <div style={{
        ...
        background:'linear-gradient(90deg, transparent, rgba(212,160,23,.07), rgba(212,160,23,.15), rgba(212,160,23,.07), transparent)',
        animation:'whScan 16s linear infinite',
      }} />
    </div>
  )
}
...
function CategoryCard({ cat, index, onClick }) {
  ...
  return (
    <div
      ...
      style={{
        position:'relative',
        background:`linear-gradient(145deg, ${c}16 0%, rgba(7,9,14,0.95) 100%)`,
        ...
        boxShadow: hov ? `0 8px 28px ${c}22` : `0 2px 8px ${c}0a`,
        animation:`whFadeUp 0.4s ${index * 0.07}s ease-out both`,
      }}
    >
      <div style={{ fontSize:34, marginBottom:12, filter:`drop-shadow(0 0 10px ${c}55)`, lineHeight:1 }}>{cat.icon}</div>
      <div style={{ fontWeight:800, fontSize:15, color:'#fff', marginBottom:8, lineHeight:1.2 }}>{cat.name}</div>
      {cat.description && (
        <div style={{ fontSize:12.5, color:'rgba(255,255,255,0.40)', lineHeight:1.6 }}>{cat.description}</div>
      )}
      <div style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        borderTop:`3px solid ${c}cc`,
        borderRadius:12,
        overflow:'hidden',
        cursor:'pointer',
        transition:'all .2s ease',
        boxShadow: hov ? `0 6px 24px ${c}1a` : 'none',
        transform: hov ? 'translateY(-4px)' : 'none',
        animation:`whFadeUp 0.35s ${(index ?? 0) * 0.06}s ease-out both`,
      }}
    >
      {page.cover_image ? (
        <img src={page.cover_image} alt={page.title} style={{ width:'100%', height:120, objectFit:'cover', display:'block' }} />
      ) : (
      ...
      <div style={{ padding:'14px 16px' }}>
        {cat && (
          <div style={{ fontSize:9, fontWeight:800, letterSpacing:'.12em', color:c, textTransform:'uppercase', marginBottom:6 }}>
            {cat.icon} {cat.name}
          </div>
        )}
...
export default function WikiHome() {
  ...
  return (
    <div style={{ minHeight:'100vh', paddingTop:80, background:'transparent', position:'relative' }}>
      <style>{WH_CSS}</style>
      <WHStars />
      <WHScanLine />
      ...
      <div style={{ position:'relative', zIndex:2 }}>
        <div style={{ maxWidth:1120, margin:'0 auto', padding:'40px 24px 100px' }}>
          ...
          <div style={{ textAlign:'center', marginBottom:52 }}>
            ...
            <p style={{ fontSize:15, color:'rgba(255,255,255,0.40)', maxWidth:520, margin:'0 auto 36px', lineHeight:1.75 }}>
            ...
            {categories.length > 0 && (
              <div style={{ display:'flex', gap:32, justifyContent:'center', flexWrap:'wrap', marginBottom:40 }}>
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontFamily:"'Pirata One', cursive", fontSize:36, fontWeight:900, color:GOLD, lineHeight:1 }}>{categories.length}</div>
                  <div style={{ fontSize:10, fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginTop:4 }}>Catégories</div>
                </div>
                <div style={{ width:1, height:40, background:'rgba(255,255,255,0.08)', alignSelf:'center' }} />
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontFamily:"'Pirata One', cursive", fontSize:36, fontWeight:900, color:GOLD, lineHeight:1 }}>{recent.length}</div>
                  <div style={{ fontSize:10, fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginTop:4 }}>Articles récents</div>
                </div>
              </div>
            )}
            ...
            <div style={{ position:'relative', maxWidth:580, margin:'0 auto 28px' }}>
              ...
              />
              {searching && (
                <div style={{ position:'absolute', right:16, top:'50%', transform:'translateY(-50%)', width:16, height:16, border:'2px solid rgba(212,160,23,0.3)', borderTopColor:GOLD, borderRadius:'50%', animation:'spin 0.75s linear infinite' }} />
              )}
            </div>
            ...
            {isAuthenticated && (
              <button
                ...
                style={{
                  ...
                  transition:'opacity .15s', letterSpacing:'.04em',
                }}
          ...
          {results === null && (
            <>
              ...
              {/* Tab separator */}
              <div style={{ height:1, background:`linear-gradient(90deg, transparent, ${activeCat?.color || GOLD}33, transparent)`, marginBottom:40, transition:'background .4s' }} />
