...
const COLOR  = '#14b8a6'
const COLOR2 = '#5eead4'
const NS     = 'rent-girlfriend'

const SYNOPSIS = "Après une rupture douloureuse, Kazuya Kinoshita loue une petite amie via une application. Sa rencontre avec Chizuru Mizuhara, une fille parfaite en apparence mais aux multiples facettes, va bouleverser sa vie et celle de son entourage dans une comédie romantique pleine de rebondissements."

const TAGS = ['Romance', 'Comédie', 'Drame', 'Slice of Life']

const AWARDS = [
  { icon: '💖', label: 'Location d\'amour' },
  { icon: '📱', label: 'L\'app qui change tout' },
  { icon: '😢', label: 'Cœur brisé & rires' },
]

const COVER = '/rent-girlfriend-poster.jpg'
...
const CSS = `
  ...
  @keyframes rgFadeUp { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:none} }
  ...
  .rg-ep-card:hover {
    ...
    box-shadow: 0 18px 48px rgba(20,184,166,.18) !important;
    border-color: rgba(20,184,166,.45) !important;
  }
  .rg-ep-card:focus-visible { outline: 2px solid ${COLOR}; outline-offset: 3px; }

  .rg-play-btn { transition: transform .16s, opacity .16s, background .16s; }
  .rg-ep-card:hover .rg-play-btn { transform: scale(1.15) !important; opacity: 1 !important; }

  .rg-cta { transition: background .18s, transform .14s, box-shadow .18s; }
  .rg-cta:hover {
    background: rgba(20,184,166,.18) !important;
    transform: translateY(-1px);
    box-shadow: 0 8px 28px rgba(20,184,166,.22);
  }

  .rg-scroll { scrollbar-width: thin; scrollbar-color: rgba(20,184,166,.2) transparent; }
  .rg-scroll::-webkit-scrollbar { width: 4px; }
  .rg-scroll::-webkit-scrollbar-thumb { background: rgba(20,184,166,.2); border-radius: 4px; }
`
...
const EpCard = memo(function EpCard({ video, index, watched, onPlay }) {
  ...
  return (
    <div className="rg-ep-card" role="button" tabIndex={0} onClick={onPlay} onKeyDown={e => e.key === 'Enter' && onPlay()}
      style={{ borderRadius: 14, overflow: 'hidden', background: 'rgba(14,12,24,.92)', border: `1px solid ${watched ? 'rgba(94,234,212,.28)' : 'rgba(255,255,255,.07)'}`, animation: `rgFadeUp .3s ${index * 0.03}s ease-out both`, position: 'relative' }}>
      <div style={{ position: 'relative', paddingTop: '57%', background: '#0a0814', overflow: 'hidden' }}>
        {video.thumbnail && !imgErr
          ...
          : <div style={{ position:'absolute',inset:0, background:`linear-gradient(135deg,rgba(20,184,166,.12),rgba(0,0,0,.9))`, display:'flex',alignItems:'center',justifyContent:'center' }}>
              <span style={{ fontFamily:"'Pirata One',cursive", fontSize:40, fontWeight:900, color:`rgba(20,184,166,.28)`, lineHeight:1 }}>{fallbackLabel}</span>
            </div>}
        <div style={{ position:'absolute',inset:'40% 0 0', background:'linear-gradient(180deg,transparent,rgba(0,0,0,.65))', pointerEvents:'none' }} />
        <div className="rg-play-btn" style={{ position:'absolute',inset:0, display:'flex',alignItems:'center',justifyContent:'center', opacity: watched ? 0.5 : 0.78 }}>
          <div style={{ width:40,height:40,borderRadius:'50%',background:`rgba(20,184,166,.82)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,backdropFilter:'blur(6px)',boxShadow:`0 4px 18px rgba(20,184,166,.45)` }}>▶</div>
        </div>
        ...
        <div style={{ position:'absolute',bottom:8,left:8, fontSize:9,fontWeight:800,background:'rgba(20,184,166,.16)',color:COLOR2,border:`1px solid rgba(20,184,166,.28)`,borderRadius:100,padding:'2px 7px' }}>{video.badge || 'VOSTFR'}</div>
      </div>
  ...
  return (
    <aside style={{ position: 'sticky', top: 0, alignSelf: 'start', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(180deg,rgba(16,12,30,.96),rgba(10,8,20,.99))', border: '1px solid rgba(20,184,166,.18)', boxShadow: '0 24px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)', backdropFilter: 'blur(20px)' }}>
      <div style={{ position:'relative', height:260, overflow:'hidden', flexShrink:0 }}>
        <img src={COVER} alt="Rent-a-Girlfriend" style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 20%',opacity:.72,filter:'saturate(1.1) brightness(.9)' }} />
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 20px' }}>
          <div style={{ fontSize:9.5,fontWeight:800,letterSpacing:'.18em',color:COLOR2,textTransform:'uppercase',marginBottom:6 }}>💖 LOCATION • CŒUR • COMÉDIE</div>
          <h2 style={{ margin:0, fontFamily:"'Pirata One',cursive", fontSize:22,fontWeight:900,color:'#fff',lineHeight:1.0,letterSpacing:'-.01em',textShadow:'0 2px 20px rgba(0,0,0,.9)' }}>Rent-a-Girlfriend</h2>
          <div style={{ fontSize:11,color:'rgba(255,255,255,.4)',fontWeight:600,marginTop:3 }}>2020</div>
        </div>
      </div>
      ...
      <div style={{ padding:'18px 18px 22px', display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <button className="rg-cta" onClick={onResume} style={{ width:'100%',padding:'11px 0',borderRadius:12, background:`rgba(20,184,166,.14)`,border:`1px solid rgba(20,184,166,.32)`, color:'#fff',cursor:'pointer',fontSize:13,fontWeight:800, display:'flex',alignItems:'center',justifyContent:'center',gap:8, fontFamily:'var(--body)' }}>
        ...
        <div style={{ display:'flex',flexWrap:'wrap',gap:5 }}>
          {TAGS.map(tag => (
            <span key={tag} style={{ fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:999,background:'rgba(20,184,166,.08)',border:'1px solid rgba(20,184,166,.2)',color:COLOR2 }}>{tag}</span>
          ))}
        </div>
...
export default function RentAGirlfriendPage({ onClose }) {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{ position:'fixed',inset:0,zIndex:500, background:'radial-gradient(circle at 18% 12%,rgba(20,184,166,.09),transparent 32rem),radial-gradient(circle at 84% 80%,rgba(94,234,212,.07),transparent 28rem),linear-gradient(135deg,#0e0a1a 0%,#100c1c 55%,#0a0814 100%)', display:'flex',flexDirection:'column' }}>
        <AnimeBackdrop motifs={ANIME_MOTIFS['rent-girlfriend']} color={COLOR} color2={COLOR2} />
        {/* Navbar */}
        <div style={{ flexShrink:0,height:62,padding:'0 24px', display:'flex',alignItems:'center',justifyContent:'space-between', background:'rgba(14,10,26,.96)',backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(20,184,166,.10)',zIndex:10, position:'relative' }}>
          ...
          <div style={{ position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:10 }}>
            <span style={{ fontSize:17,animation:'rgFloat 6s ease-in-out infinite' }}>💖</span>
            <span style={{ fontFamily:"'Pirata One',cursive",fontSize:17,fontWeight:900,color:'#fff',letterSpacing:'-.01em' }}>
              {playerIdx !== null ? (VIDEOS[playerIdx]?.kind === 'film' || VIDEOS[playerIdx]?.kind === 'ova' ? VIDEOS[playerIdx]?.title : `Épisode ${VIDEOS[playerIdx]?.episode}`) : 'Rent-a-Girlfriend'}
            </span>
          </div>
            ...
            <div className="rg-layout">
              ...
              <div>
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20 }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:999,background:'rgba(20,184,166,.08)',border:'1px solid rgba(20,184,166,.18)' }}>
                ...
                <div style={{ padding: '24px', borderRadius: 16, background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.06)', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 900, color: '#fff', marginBottom: 8 }}>Rent-a-Girlfriend</div>
                  <div style={{ color: 'rgba(255,255,255,.5)', marginBottom: 16 }}>La comédie romantique de Reiji Miyajima arrive avec une expérience dédiée bientôt.</div>
                </div>
                ...
                <div style={{ marginTop:28,padding:'14px 18px',borderRadius:12,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',gap:10 }}>
                  ...
                  <span style={{ fontSize:12,color:'rgba(255,255,255,.38)',fontWeight:600,lineHeight:1.5 }}>
                    Rent-a-Girlfriend — quand la location d'une petite amie change tout.
                  </span>
                </div>
              </div>
            </div>
