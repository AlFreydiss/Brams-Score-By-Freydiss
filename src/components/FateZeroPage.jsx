...
import VIDEOS_RAW from '../data/fate-zero-videos.json'
...
const SYNOPSIS = "Dix ans avant Fate/stay night, sept mages invoquent sept Servants pour s'affronter dans la 4e Guerre du Saint-Graal — un combat à mort dont le vainqueur voit n'importe quel vœu exaucé. Kiritsugu Emiya, le « tueur de mages », et Saber, le Roi des Chevaliers, s'allient dans une guerre de l'ombre faite d'idéaux brisés et de tragédies."

const TAGS = ['Action', 'Surnaturel', 'Drame', 'Seinen', 'ufotable']

const AWARDS = [
  { icon: '⚔️', label: '4e Guerre du Saint-Graal' },
  { icon: '👑', label: 'Saber, Roi des Chevaliers' },
  { icon: '🏆', label: "Chef-d'œuvre ufotable" },
]
...
const CSS = `
  ...
  .aot-ep-card:hover {
    ...
    box-shadow: 0 18px 48px rgba(244,63,94,.18) !important;
  ...
  .aot-cta:hover {
    ...
    box-shadow: 0 8px 28px rgba(244,63,94,.22);
  }

  .aot-scroll { scrollbar-width: thin; scrollbar-color: rgba(244,63,94,.2) transparent; }
  .aot-scroll::-webkit-scrollbar { width: 4px; }
  .aot-scroll::-webkit-scrollbar-thumb { background: rgba(244,63,94,.2); border-radius: 4px; }
`
...
const EpCard = memo(function EpCard({ video, index, watched, onPlay }) {
  ...
  return (
    <div className="aot-ep-card" role="button" tabIndex={0} onClick={onPlay} onKeyDown={e => e.key === 'Enter' && onPlay()}
      style={{ borderRadius: 14, overflow: 'hidden', background: 'rgba(14,12,24,.92)', border: `1px solid ${watched ? 'rgba(251,113,133,.28)' : 'rgba(255,255,255,.07)'}`, animation: `aotFadeUp .3s ${index * 0.03}s ease-out both`, position: 'relative' }}>
      <div style={{ position: 'relative', paddingTop: '57%', background: '#0a0814', overflow: 'hidden' }}>
        {video.thumbnail && !imgErr
          ...
          : <div style={{ position:'absolute',inset:0, background:`linear-gradient(135deg,rgba(244,63,94,.12),rgba(0,0,0,.9))`, display:'flex',alignItems:'center',justifyContent:'center' }}>
              <span style={{ fontFamily:"'Pirata One',cursive", fontSize:40, fontWeight:900, color:`rgba(244,63,94,.28)`, lineHeight:1 }}>{fallbackLabel}</span>
            </div>}
        ...
        <div className="aot-play-btn" style={{ position:'absolute',inset:0, display:'flex',alignItems:'center',justifyContent:'center', opacity: watched ? 0.5 : 0.78 }}>
          <div style={{ width:40,height:40,borderRadius:'50%',background:`rgba(244,63,94,.82)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,backdropFilter:'blur(6px)',boxShadow:`0 4px 18px rgba(244,63,94,.45)` }}>▶</div>
        </div>
        ...
        <div style={{ position:'absolute',bottom:8,left:8, fontSize:9,fontWeight:800,background:'rgba(244,63,94,.16)',color:COLOR2,border:`1px solid rgba(244,63,94,.28)`,borderRadius:100,padding:'2px 7px' }}>{video.badge || 'VOSTFR'}</div>
      </div>
  ...
  return (
    <aside className="anime-infopanel" style={{ position: 'sticky', top: 0, alignSelf: 'start', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(180deg,rgba(20,16,8,.96),rgba(12,10,6,.99))', border: `1px solid ${COLOR}33`, boxShadow: '0 24px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)', backdropFilter: 'blur(20px)' }}>
      <div style={{ position:'relative', height:260, overflow:'hidden', flexShrink:0, background:`radial-gradient(ellipse at 50% 30%, ${COLOR}44, #0c0a06 72%)` }}>
        {COVER && <img loading="lazy" decoding="async" src={COVER} alt="Fate/Zero" style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 15%',opacity:.72,filter:'saturate(1.1) brightness(.88)' }} />}
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 20px' }}>
          <div style={{ fontSize:9.5,fontWeight:800,letterSpacing:'.18em',color:COLOR2,textTransform:'uppercase',marginBottom:6 }}>⚔️ SAINT-GRAAL • SERVANTS • MAGES</div>
          <h2 style={{ margin:0, fontFamily:"'Pirata One',cursive", fontSize:22,fontWeight:900,color:'#fff',lineHeight:1.0,letterSpacing:'-.01em',textShadow:'0 2px 20px rgba(0,0,0,.9)' }}>Fate/Zero</h2>
          <div style={{ fontSize:11,color:'rgba(255,255,255,.4)',fontWeight:600,marginTop:3 }}>ufotable · 2011</div>
        </div>
      </div>
      ...
      <div style={{ padding:'18px 18px 22px', display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <button className="aot-cta" onClick={onResume} style={{ width:'100%',padding:'11px 0',borderRadius:12, background:`rgba(244,63,94,.14)`,border:`1px solid rgba(244,63,94,.32)`, color:'#fff',cursor:'pointer',fontSize:13,fontWeight:800, display:'flex',alignItems:'center',justifyContent:'center',gap:8, fontFamily:'var(--body)' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
          {[
            ...
            { label:'Saisons', value:'2', dot:'#fbbf24' },
            { label:'Studio', value:'ufotable', dot:'#34d399' },
        ...
        <div style={{ display:'flex',flexWrap:'wrap',gap:5 }}>
          {TAGS.map(tag => (
            <span key={tag} style={{ fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:999,background:'rgba(244,63,94,.08)',border:'1px solid rgba(244,63,94,.2)',color:COLOR2 }}>{tag}</span>
          ))}
        </div>
...
export default function FateZeroPage({ onClose }) {
        ...
        <div style={{ flexShrink:0,height:62,padding:'0 24px', display:'flex',alignItems:'center',justifyContent:'space-between', background:'rgba(14,10,26,.96)',backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(244,63,94,.10)',zIndex:10, position:'relative' }}>
          ...
          <div style={{ position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:10 }}>
            ...
            <span style={{ fontFamily:"'Pirata One',cursive",fontSize:17,fontWeight:900,color:'#fff',letterSpacing:'-.01em' }}>
              {detailIdx !== null ? (VIDEOS[detailIdx]?.title || `Épisode ${VIDEOS[detailIdx]?.episode}`) : "Fate/Zero"}
            </span>
          </div>
        ...
        {detailIdx !== null ? (
          ...
          <div ref={scrollRef} className="aot-scroll" style={{ flex:1,overflowY:'auto',padding:'24px 28px 48px' }}>
            ...
            <div className="aot-layout">
              <InfoPanel watchedCount={watchedCount} total={VIDEOS.length} lastWatchedIdx={resumeIdx} onResume={() => openDetail(resumeIdx)} episodeCount={episodes.length} />
              <div>
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20 }}>
                  <div>
                    ...
                    <div style={{ fontSize:11,color:'rgba(255,255,255,.32)',fontWeight:600 }}>
                      {episodes.length > 0 ? `${episodes.length} épisodes • VOSTFR • 1080p` : 'Bientôt disponible'}
                    </div>
                  </div>
                ...
                {episodes.length > 0 ? (
                  <>
                    ...
                    <div style={{ marginTop:6,padding:'14px 18px',borderRadius:12,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',gap:10 }}>
                      ...
                      <span style={{ fontSize:12,color:'rgba(255,255,255,.38)',fontWeight:600,lineHeight:1.5 }}>
                        Fate/Zero (2011) — la 4e Guerre du Saint-Graal, par ufotable, sur un récit de Gen Urobuchi. Préquelle de Fate/stay night.
                      </span>
                    </div>
                  </>
                ) : (
                  <>
                    <div style={{ padding: '24px', borderRadius: 16, background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.06)', textAlign: 'center' }}>
                      ...
                      <div style={{ color: 'rgba(255,255,255,.5)', marginBottom: 16 }}>Fate/Zero arrive très bientôt en 1080p VOSTFR sur le site.</div>
                    ...
                    <div style={{ marginTop:28,padding:'14px 18px',borderRadius:12,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',gap:10 }}>
                      <span style={{ fontSize:16 }}>⚔️</span>
                      <span style={{ fontSize:12,color:'rgba(255,255,255,.38)',fontWeight:600,lineHeight:1.5 }}>
                        Fate/Zero (2011) — chef-d'œuvre ufotable, préquelle de Fate/stay night.
                      </span>
                    </div>
                  </>
                )}
