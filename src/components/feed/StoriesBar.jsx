...
export default function StoriesBar() {
  ...
  const Ring = ({ children }) => (
    <div style={{ padding: 2, borderRadius: '50%', background: 'linear-gradient(135deg, #d4a017, #9b6cff)' }}>
    ...
    <div style={{ display: 'flex', gap: 14, padding: '14px 16px', borderBottom: `1px solid ${T.border}`, overflowX: 'auto' }}>
      ...
      {authors.map((a, i) => (
        <button key={a.author_id} onClick={() => setViewerIdx(i)}
          style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, border: 'none', background: 'transparent', cursor: 'pointer', flexShrink: 0, width: 64 }}>
          <Ring><span style={avatar(54)}>{a.avatar ? <img src={a.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (a.username || '?').slice(0, 2).toUpperCase()}</span></Ring>
