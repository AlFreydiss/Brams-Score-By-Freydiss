...
const SYNOPSIS = "Dans un monde où l'humanité vit terrée derrière d'immenses murailles pour échapper aux Titans, Eren Yeager jure de les exterminer tous après la chute du mur Maria. Une épopée de liberté, de sacrifice et de révélations sur l'histoire du monde."

const TAGS = ['Action', 'Drame', 'Surnaturel', 'Guerre', 'Mystery']

const AWARDS = [
  { icon: '🗡️', label: 'Épopée de la liberté' },
  { icon: '🧱', label: 'Les 3 Murailles' },
  { icon: '🔥', label: 'Révélation ultime' },
]

const COVER = '/aot-poster.jpg'
...
function InfoPanel({ watchedCount, total, lastWatchedIdx, onResume, chapterCount, episodeCount }) {
  ...
  return (
    <aside style={{ position: 'sticky', top: 0, alignSelf: 'start', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(180deg,rgba(16,12,30,.96),rgba(10,8,20,.99))', border: '1px solid rgba(244,63,94,.18)', boxShadow: '0 24px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)', backdropFilter: 'blur(20px)' }}>
      <div style={{ position:'relative', height:260, overflow:'hidden', flexShrink:0 }}>
        <img loading="lazy" decoding="async" src={COVER} alt="L'Attaque des Titans" style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 15%',opacity:.72,filter:'saturate(1.1) brightness(.88)' }} />
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 20px' }}>
          <div style={{ fontSize:9.5,fontWeight:800,letterSpacing:'.18em',color:COLOR2,textTransform:'uppercase',marginBottom:6 }}>🗡️ LIBERTÉ • TITANS • VÉRITÉ</div>
          <h2 style={{ margin:0, fontFamily:"'Pirata One',cursive", fontSize:22,fontWeight:900,color:'#fff',lineHeight:1.0,letterSpacing:'-.01em',textShadow:'0 2px 20px rgba(0,0,0,.9)' }}>L'Attaque des Titans</h2>
          <div style={{ fontSize:11,color:'rgba(255,255,255,.4)',fontWeight:600,marginTop:3 }}>MAPPA · 2013</div>
        </div>
      </div>
...
export default function AotPage({ onClose }) {
  ...
  const chapterCount = CHAPTERS.length || 81
        ...
        <div style={{ flexShrink:0,height:62,padding:'0 24px', display:'flex',alignItems:'center',justifyContent:'space-between', background:'rgba(14,10,26,.96)',backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(244,63,94,.10)',zIndex:10, position:'relative' }}>
          ...
          <div style={{ position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:10 }}>
            ...
            <span style={{ fontFamily:"'Pirata One',cursive",fontSize:17,fontWeight:900,color:'#fff',letterSpacing:'-.01em' }}>
              {detailIdx !== null ? (VIDEOS[detailIdx]?.title || `Épisode ${VIDEOS[detailIdx]?.episode}`) : "L'Attaque des Titans"}
            </span>
          </div>
        ...
        {detailIdx !== null ? (
          ...
          <div ref={scrollRef} className="aot-scroll" style={{ flex:1,overflowY:'auto',padding:'24px 28px 48px' }}>
            ...
            <div className="aot-layout">
              <InfoPanel watchedCount={watchedCount} total={VIDEOS.length} lastWatchedIdx={resumeIdx} onResume={() => openDetail(resumeIdx)} chapterCount={chapterCount} episodeCount={episodes.length} />
              <div>
