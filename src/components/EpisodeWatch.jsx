    ...
    <div className="ew-root" style={{ position: 'relative', marginBottom: 10, animation: 'ewFade .3s ease-out both' }}>
      <style>{`
        ...
        /* ===== Tablette (≤1024px) : lecteur plus grand, détail empilé dessous =====
           NON-sticky (demande Freydiss) : le lecteur reste à sa place et défile
           normalement avec la page — il ne « suit » plus le scroll. Pour vraiment
           regarder = plein écran. */
        @media (max-width:1024px){
          .ew-grid{ grid-template-columns:1fr; gap:18px; }
          .ew-player{
            position:static; border-radius:14px; background:#000;
            box-shadow:0 10px 28px rgba(0,0,0,.55);
          }
        }

        /* ===== Mobile (≤768px) : lecteur edge-to-edge proéminent, NON-sticky ===== */
        @media (max-width:768px){
          ...
          .ew-grid{ gap:0; }
          .ew-player{
            position:static; width:100vw; border-radius:0;
            border-left:0; border-right:0; border-top:0; background:#000;
          }
