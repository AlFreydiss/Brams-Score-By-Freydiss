...
function StatBlock({ value, label, icon, live = false, liveVal = null }) {
  ...
  return (
    <div
      ...
      style={{
        background: hov ? 'rgba(212,160,23,0.07)' : 'rgba(255,255,255,0.03)',
        border: `1px solid ${hov ? 'rgba(212,160,23,0.25)' : 'rgba(255,255,255,0.07)'}`,
        borderRadius: 14, padding: '16px 20px', transition: 'all .2s ease',
        transform: hov ? 'translateY(-3px)' : 'none',
        boxShadow: hov ? '0 8px 28px rgba(212,160,23,0.12)' : 'none',
        cursor: 'default',
      }}
    >
      {icon && <div style={{ fontSize: 18, marginBottom: 8, opacity: 0.8 }}>{icon}</div>}
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 1280, margin: '0 auto', padding: '0 40px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1fr) minmax(0,380px)', alignItems: 'start', gap: isMobile ? 48 : 64 }}>
          ...
          <div>
            ...
            {/* Stats */}
            <div className="fade-up-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px,1fr))', gap: 12, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,.07)' }}>
