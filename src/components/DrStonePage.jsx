...
import VIDEOS_RAW from '../data/drstone-videos.json'
...
const SYNOPSIS = "Après une mystérieuse lumière qui a pétrifié toute l'humanité en pierre pendant 3700 ans, Senku Ishigami se réveille dans un monde redevenu primitif. Armé de son génie scientifique, il décide de reconstruire la civilisation depuis zéro, en réinventant la science, la technologie et l'industrie pour ramener l'humanité à l'ère moderne."

const TAGS = ['Science', 'Aventure', 'Shōnen', 'Post-apocalyptique', 'TMS']

const AWARDS = [
  { icon: '🧪', label: 'Science avant tout' },
  { icon: '🪨', label: '3700 ans de plans' },
  { icon: '🔬', label: 'Le génie de Senku' },
]

const COVER = '/drstone-poster.jpg'
...
const CSS = `
  ...
  @keyframes dsPulse { 0%,100%{opacity:.6} 50%{opacity:1} }
  @keyframes dsFadeUp { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:none} }
  ...
  .ds-ep-card:hover {
    ...
    box-shadow: 0 18px 48px rgba(0,184,148,.18) !important;
    border-color: rgba(0,184,148,.45) !important;
  }
  .ds-ep-card:focus-visible { outline: 2px solid ${COLOR}; outline-offset: 3px; }

  .ds-play-btn { transition: transform .16s, opacity .16s, background .16s; }
  .ds-ep-card:hover .ds-play-btn { transform: scale(1.15) !important; opacity: 1 !important; }

  .ds-cta { transition: background .18s, transform .14s, box-shadow .18s; }
  .ds-cta:hover {
    background: rgba(0,184,148,.18) !important;
    transform: translateY(-1px);
    box-shadow: 0 8px 28px rgba(0,184,148,.22);
  }

  .ds-scroll { scrollbar-width: thin; scrollbar-color: rgba(0,184,148,.2) transparent; }
  .ds-scroll::-webkit-scrollbar { width: 4px; }
  .ds-scroll::-webkit-scrollbar-thumb { background: rgba(0,184,148,.2); border-radius: 4px; }
`
...
const EpCard = memo(function EpCard({ video, index, watched, onPlay }) {
  ...
  return (
    <div className="ds-ep-card" role="button" tabIndex={0} onClick={onPlay} onKeyDown={e => e.key === 'Enter' && onPlay()}
      style={{ borderRadius: 14, overflow: 'hidden', background: 'rgba(14,12,24,.92)', border: `1px solid ${watched ? 'rgba(85,239,196,.28)' : 'rgba(255,255,255,.07)'}`, animation: `dsFadeUp .3s ${index * 0.04}s ease-out both`, position: 'relative' }}>
      <div style={{ position: 'relative', paddingTop: '57%', background: '#0a0814', overflow: 'hidden' }}>
        {video.thumbnail && !imgErr
          ...
          : <div style={{ position:'absolute',inset:0, background:`linear-gradient(135deg,rgba(0,184,148,.12),rgba(0,0,0,.9))`, display:'flex',alignItems:'center',justifyContent:'center' }}>
              <span style={{ fontFamily:"'Pirata One',cursive", fontSize:40, fontWeight:900, color:`rgba(0,184,148,.28)`, lineHeight:1 }}>{fallbackLabel}</span>
            </div>}
        <div style={{ position:'absolute',inset:'40% 0 0', background:'linear-gradient(180deg,transparent,rgba(0,0,0,.65))', pointerEvents:'none' }} />
        <div className="ds-play-btn" style={{ position:'absolute',inset:0, display:'flex',alignItems:'center',justifyContent:'center', opacity: watched ? 0.5 : 0.78 }}>
          <div style={{ width:40,height:40,borderRadius:'50%',background:`rgba(0,184,148,.82)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,backdropFilter:'blur(6px)',boxShadow:`0 4px 18px rgba(0,184,148,.45)` }}>▶</div>
        </div>
        ...
        <div style={{ position:'absolute',bottom:8,left:8, fontSize:9,fontWeight:800,background:'rgba(0,184,148,.16)',color:COLOR2,border:`1px solid rgba(0,184,148,.28)`,borderRadius:100,padding:'2px 7px' }}>{video.badge || 'VOSTFR'}</div>
      </div>
    ...
    <aside style={{ position: 'sticky', top: 0, alignSelf: 'start', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(180deg,rgba(16,12,30,.96),rgba(10,8,20,.99))', border: '1px solid rgba(0,184,148,.18)', boxShadow: '0 24px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)', backdropFilter: 'blur(20px)' }}>
      <div style={{ position:'relative', height:260, overflow:'hidden', flexShrink:0 }}>
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 20px' }}>
          ...
          <h2 style={{ margin:0, fontFamily:"'Pirata One',cursive", fontSize:22,fontWeight:900,color:'#fff',lineHeight:1.0,letterSpacing:'-.01em',textShadow:'0 2px 20px rgba(0,0,0,.9)' }}>Dr. Stone</h2>
          <div style={{ fontSize:11,color:'rgba(255,255,255,.4)',fontWeight:600,marginTop:3 }}>TMS Entertainment · 2019</div>
        </div>
      </div>
      ...
      <div style={{ padding:'18px 18px 22px', display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <div style={{ display:'flex',flexWrap:'wrap',gap:5 }}>
          {TAGS.map(tag => (
            <span key={tag} style={{ fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:999,background:'rgba(0,184,148,.08)',border:'1px solid rgba(0,184,148,.2)',color:COLOR2 }}>{tag}</span>
          ))}
        </div>
...
export default function DrStonePage({ onClose }) {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{ position:'fixed',inset:0,zIndex:500, background:'radial-gradient(circle at 18% 12%,rgba(0,184,148,.09),transparent 32rem),radial-gradient(circle at 84% 80%,rgba(85,239,196,.07),transparent 28rem),linear-gradient(135deg,#0e0a1a 0%,#100c1c 55%,#0a0814 100%)', display:'flex',flexDirection:'column' }}>
        <AnimeBackdrop motifs={ANIME_MOTIFS.drstone} color={COLOR} color2={COLOR2} />
        {/* Navbar */}
        <div style={{ flexShrink:0,height:62,padding:'0 24px', display:'flex',alignItems:'center',justifyContent:'space-between', background:'rgba(14,10,26,.96)',backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(0,184,148,.10)',zIndex:10, position:'relative' }}>
          ...
          <div style={{ position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:10 }}>
            <span style={{ fontSize:17,animation:'dsFloat 6s ease-in-out infinite' }}>⚗️</span>
            <span style={{ fontFamily:"'Pirata One',cursive",fontSize:17,fontWeight:900,color:'#fff',letterSpacing:'-.01em' }}>
              {playerIdx !== null ? (VIDEOS[playerIdx]?.kind === 'film' || VIDEOS[playerIdx]?.kind === 'ova' ? VIDEOS[playerIdx]?.title : `Épisode ${VIDEOS[playerIdx]?.episode}`) : 'Dr. Stone'}
            </span>
          </div>
        ...
        {playerIdx !== null ? (
          ...
          <div ref={scrollRef} className="ds-scroll" style={{ flex:1,overflowY:'auto',padding:'24px 28px 48px' }}>
            <style>{`
              .ds-layout { display: grid; grid-template-columns: 310px minmax(0,1fr); gap: 28px; max-width: 1480px; margin: 0 auto; align-items: start; }
              ...
              <div>
                ...
                <div style={{ marginTop:28,padding:'14px 18px',borderRadius:12,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',gap:10 }}>
                  ...
                  <span style={{ fontSize:12,color:'rgba(255,255,255,.38)',fontWeight:600,lineHeight:1.5 }}>
                    Dr. Stone est une production TMS Entertainment (2019), une aventure scientifique unique où la science reconstruit le monde.
                  </span>
                </div>
              </div>
            </div>
