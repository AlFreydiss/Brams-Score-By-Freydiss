...
SOURCE = Path(r'F:\Manga\TPN\One Piece')
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            try:
                ...
                failed.append((sp, lp))
                print(f'  [FAIL] {sp} — {e}')
    print(f'[OP] Terminé : {total - len(failed)}/{total} OK, {len(failed)} échecs')
    if failed:
...
json_data = [
    {
        'num': ch['num_raw'],
        'title': ch['title'],
        'pages': [f"{BASE_URL}/{ch['storage_prefix']}/{p.name}" for p in ch['pages']],
    }
    for ch in chapters
]
