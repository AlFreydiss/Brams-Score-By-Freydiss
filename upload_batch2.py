...
SOURCES = {
    'kny': Path(r'F:\Manga\TPN\Kimetsu No Yaiba'),
    'nnt': Path(r'F:\Manga\TPN\Nanatsu no Taizai'),
    'sl':  Path(r'F:\Manga\TPN\Solo Leveling'),
    'dbs': Path(r'F:\Manga\TPN\Dragon Ball Super'),
    'bc':  Path(r'F:\Manga\TPN\Black Clover'),
}
...
def scan_chapters(manga_key, base_dir):
    ...
    for folder in sorted(base_dir.iterdir()):
        ...
        if num_raw is None:
            print(f'[SKIP] {folder.name}'); continue
        pages = sorted(p for p in folder.iterdir() if p.is_file() and p.suffix.lower() in IMG_EXTS)
        if not pages: continue
        chapters.append({'num_raw': num_raw, 'num': float(num_raw), 'title': title, 'pages': pages, 'storage_prefix': f'{manga_key}/ch{num_raw}'})
...
def upload_all(manga_key, chapters):
    ...
    if failed:
        print(f'  Relance {len(failed)} échecs...')
        still = []
        for sp, lp in failed:
            try: upload_file(sp, lp); print(f'    [RETRY OK] {sp}')
            except Exception as e: still.append(sp); print(f'    [RETRY FAIL] {sp} — {e}')
        if still: print(f'  Encore en échec : {still}')

BASE_URL = f'{SUPABASE_URL}/storage/v1/object/public/{BUCKET}'

for manga_key, base_dir in SOURCES.items():
    if not base_dir.exists():
        print(f'[WARN] Dossier introuvable : {base_dir}'); continue
    print(f'\n=== {manga_key.upper()} ===')
