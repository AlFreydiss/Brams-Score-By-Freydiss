/**
 * Your Name (Kimi no Na wa) — Le Film : upload VO (JA) + VF + sous-titres FR
 * + miniature sur R2.
 * Source (générée via ffmpeg depuis le MKV BDRip, voir pipeline du film) :
 *   F:\Brams-Score-By-Freydiss-new\public\anime\your-name\
 *     the-movie.mp4        (vidéo h264 + audio JA AAC)  → VO / défaut
 *     the-movie.vf.mp4     (vidéo h264 + audio VF AAC)
 *     the-movie.fr.vtt     (sous-titres VOSTFR ASS→VTT)
 *     the-movie-thumb.jpg  (frame extraite au milieu, 480x270)
 * Destination R2 : anime/your-name/<fichier>
 */
...
const SOURCE_DIR = process.env.YN_MOVIE_DIR
  || 'F:\\Brams-Score-By-Freydiss-new\\public\\anime\\your-name'
const PREFIX = 'anime/your-name'
...
const FILES = [
  ...
  { local: 'the-movie-thumb.jpg', key: `${PREFIX}/the-movie-thumb.jpg`, type: 'image/jpeg' },
]
...
async function uploadFile({ local, key, type }) {
  ...
  if (!fs.existsSync(filePath)) { console.log(`⚠ absent, skip : ${local}`); return }
