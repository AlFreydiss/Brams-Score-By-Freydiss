...
SOURCES = {
    'drstone': Path(r'F:\Manga\TPN\Dr. Stone'),
    'jjk':     Path(r'F:\Manga\TPN\Jujutsu Kaisen'),
}
...
def upload_all(manga_key, chapters):
    ...
    for ch in chapters:
        for p in ch['pages']:
            storage_path = f"{ch['storage_prefix']}/{p.name}"
            tasks.append((storage_path, p))
...
def build_json(manga_key, chapters, base_url):
    result = []
    for ch in chapters:
        urls = []
        for p in ch['pages']:
            urls.append(f"{base_url}/{ch['storage_prefix']}/{p.name}")
        result.append({
            'num':   ch['num_raw'],
            'title': ch['title'],
            'pages': urls,
