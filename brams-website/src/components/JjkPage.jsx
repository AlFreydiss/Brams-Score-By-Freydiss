...
import JJK_CHAPTERS_DATA from '../data/jjk-chapters.json'
import JJK_VIDEOS_DATA from '../data/jjk-videos.json'
import { Reader } from './MangaReader.jsx'

const JJK_COLOR = '#c62828'

const EMOJIS = ['⚡','💀','🔮','👊','🌀','🌑','🦴','🔱','⚔️','🕷️','🌊','🔥','💢','👁️','🌫️','🧿','⛓️','🩸','🌪️','👹','💣','🌙','🗡️','🔴','🎭','🌒','🦂']

const CHAPTERS = JJK_CHAPTERS_DATA.map((ch, i) => ({
  num:   ch.num,
...
const VIDEOS = JJK_VIDEOS_DATA

function loadProgress() {
  try { return JSON.parse(localStorage.getItem('jjk_progress') || '{}') } catch { return {} }
}
function saveProgress(p) {
  try { localStorage.setItem('jjk_progress', JSON.stringify(p)) } catch {}
}
...
function ChapterCard({ ch, status, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        position: 'relative',
        background: hovered ? `${JJK_COLOR}18` : isRead ? 'rgba(20,21,24,0.5)' : 'rgba(20,21,24,0.85)',
        border: isReading ? `2px solid ${JJK_COLOR}` : `1px solid ${hovered ? JJK_COLOR + '55' : 'rgba(255,255,255,0.07)'}`,
        borderRadius: 14, padding: '16px',
        cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--body)',
        transition: 'all 0.18s ease',
        transform: hovered ? 'translateY(-3px)' : 'translateY(0)',
        boxShadow: hovered ? `0 10px 28px ${JJK_COLOR}18` : 'none',
    ...
    >
      ...
      {isReading && (
        <div style={{ position: 'absolute', top: 10, right: 10, fontSize: 10, fontWeight: 700, background: `${JJK_COLOR}22`, color: JJK_COLOR, border: `1px solid ${JJK_COLOR}55`, borderRadius: 100, padding: '2px 8px' }}>En cours</div>
      )}
      <div style={{ fontSize: 22, marginBottom: 10 }}>{ch.emoji}</div>
      <div style={{ fontSize: 10, fontWeight: 700, color: JJK_COLOR, letterSpacing: '0.08em', marginBottom: 4 }}>CHAPITRE {ch.num}</div>
      <div style={{ fontSize: 13, fontWeight: 700, color: isRead ? 'rgba(255,255,255,0.5)' : '#fff', lineHeight: 1.35, marginBottom: 10 }}>{ch.title}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: hovered ? JJK_COLOR : 'rgba(255,255,255,0.3)', transition: 'color 0.18s' }}>
...
function VideoCard({ video, isPlaying, onPlay }) {
  ...
  return (
    <div
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{ borderRadius: 16, overflow: 'hidden', background: 'rgba(18,19,22,0.9)', border: `1px solid ${hovered ? JJK_COLOR + '40' : 'rgba(255,255,255,0.07)'}`, transition: 'all 0.2s', transform: hovered ? 'translateY(-4px)' : 'translateY(0)' }}
    >
      {isPlaying ? (
        ...
        <div onClick={onPlay} style={{ position: 'relative', paddingTop: '56.25%', background: '#0a0b0d', cursor: 'pointer', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: 58, height: 58, borderRadius: '50%', background: `${JJK_COLOR}cc`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, backdropFilter: 'blur(4px)' }}>▶</div>
          </div>
        </div>
      )}
      <div style={{ padding: '14px 16px' }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: JJK_COLOR, letterSpacing: '0.07em', marginBottom: 5 }}>ÉPISODE {video.episode}</div>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'var(--bg)', display: 'flex', flexDirection: 'column', animation: 'fadeIn 0.18s ease-out' }}>
        <div style={{ flexShrink: 0, background: 'rgba(17,18,20,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)', zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 20px', height: 64 }}>
            ...
            >← Retour</button>
            <div style={{ width: 3, height: 32, borderRadius: 2, background: JJK_COLOR, flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'var(--display)', fontWeight: 800, fontSize: 16, color: '#fff' }}>⚡ Jujutsu Kaisen</div>
            ...
            <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border)', flexShrink: 0 }}>
              {[['scans', '📖 Scans'], ['videos', '🎬 Épisodes']].map(([t, label]) => (
                <button key={t} onClick={() => setTab(t)} style={{ height: 38, padding: '0 18px', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700, background: tab === t ? `${JJK_COLOR}28` : 'transparent', color: tab === t ? JJK_COLOR : 'var(--muted)', borderRight: t === 'scans' ? '1px solid var(--border)' : 'none', transition: 'all 0.15s' }}>{label}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 20px' }}>
          <div style={{ maxWidth: 1120, margin: '0 auto' }}>
            {tab === 'scans' ? (
              CHAPTERS.length === 0 ? (
                <EmptyState icon="⚡" title="Scans bientôt disponibles" desc="Les chapitres de Jujutsu Kaisen seront ajoutés prochainement." />
              ) : (
