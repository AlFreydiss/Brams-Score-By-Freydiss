...
const RANKS = [
  ...
  { emoji:'👑',  name:'Yonkou',          hours:'70h',  color:'#9B59B6', desc:'Élite du serveur. Une des grandes puissances du Brams.' },
...
export default function Ranks() {
  ...
  return (
    <section id="rangs" style={{ background:'var(--surface)', position:'relative' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:64 }}>
          <div className={`reveal ${inView?'visible':''}`}>
            ...
            <p className="sub" style={{ margin:'0 auto' }}>
              Plus tu passes de temps en vocal, plus tu montes. Les rangs se cumulent — et ça se voit.
            </p>
          </div>
        </div>
          ...
          {RANKS.map((r, i) => (
            <div key={r.name} className={`reveal reveal-${Math.min(i+1,4)} ${inView?'visible':''}`}
              style={{
                ...
                background:'var(--card)', borderRadius:14, padding:'20px 26px',
                border:`1px solid ${r.color}18`, transition:'transform .2s, box-shadow .2s', cursor:'default',
              }}
              onMouseEnter={e=>{ e.currentTarget.style.transform='translateX(8px)'; e.currentTarget.style.boxShadow=`0 6px 28px ${r.color}18` }}
            ...
            >
              ...
              <div style={{ width:10, height:10, borderRadius:'50%', background:r.color, boxShadow:`0 0 10px ${r.color}`, flexShrink:0 }} />
            </div>
          ))}
        </div>
